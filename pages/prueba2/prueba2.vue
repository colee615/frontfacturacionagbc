   ...
   <div>
      <form @submit.prevent="calculate">
         <div>
            ...
            <select v-model="servicio" id="servicio" @change="filterProvincias" required>
         ...
         <div>
            ...
            <select v-model="provincia" id="provincia" required>
               <option v-for="provincia in provinciasFiltradas" :key="provincia" :value="provincia">
                  {{ provincia }}
               </option>
            </select>
         </div>
         <div>
            ...
            <input v-model="peso" type="number" id="peso" required>
         </div>
         <div>
            <label for="cantidad">Cantidad:</label>
            <input v-model="cantidad" type="number" id="cantidad" required>
         </div>
         <div>
         ...
         <div>
            ...
            <input v-model="telefonodestinatario" type="text" id="telefonodestinatario">
         </div>
         <button type="submit">Calcular</button>
      </form>
      <div v-if="importe !== null">
         <p>Importe calculado: {{ importe }}</p>
      </div>
   </div>
</template>
...
export default {
   data() {
      return {
         ...
         provincias: [
            ...
            'LOCAL 4',
            'CUIDAD CAPITAL EMS',
            'CUIDAD INTERMEDIA EMS',
            'TRINIDAD/COBIJA EMS',
            'RIBERALTA/GUAYARAMERIN EMS',
            'CUIDAD CAPITAL ME',
            'TRINIDAD/COBIJA ME',
            'PROVINCIA-DENTRO ME',
            'PROVINCIA-OTRO ME',
            'SERVICIO-LOCAL LC/AO',
            'SERVICIO-NACIONAL LC/AO',
            'PROVINCIA-DENTRO LC/AO',
            'PROVINCIA-OTRO LC/AO',
            'TRINIDAD/COBIJA LC/AO',
            'RIBERALTA/GUAYARAMERIN LC/AO',
            'SERVICIO-LOCAL ECA',
            'SERVICIO-NACIONAL ECA',
            'PROVINCIA-DENTRO ECA',
            'PROVINCIA-OTRO ECA',
            'TRINIDAD/COBIJA ECA',
            'RIBERALTA/GUAYARAMERIN ECA',
            'UNICO SE',
            'SERVICIO-LOCAL PO',
            'SERVICIO-NACIONAL PO',
            'PROVINCIA-DENTRO PO',
            'PROVINCIA-OTRO PO',
            'SERVICIO-NACIONAL SM',
            'SERVICIO-PROVINCIONAL SM'
         ],
         provinciasFiltradas: []
      };
   },
   methods: {
      filterProvincias() {
         if (this.servicio) {
            this.provinciasFiltradas = this.provincias.filter(provincia => provincia.includes(this.servicio));
         } else {
            this.provinciasFiltradas = this.provincias;
         }
      },
      calcularPrecio(provincia, peso, tarifas) {
         provincia = provincia.toUpperCase();
         if (tarifas.hasOwnProperty(provincia)) {
            return tarifas[provincia];
         }
         return 0;
      },
      calcularImporte(provincia, peso, cantidad, telefonodestinatario) {
         let importe = 0;

         const tarifasPorPeso = {
            '0.001-0.020': {
               'LOCAL 1': 3,
               'LOCAL 2': 4,
               'LOCAL 3': 5,
         ...
         const tarifa = Object.keys(tarifasPorPeso).find(rango => {
            const [min, max] = rango.split('-').map(parseFloat);
            return peso >= min && peso <= max;
         });

         if (tarifa) {
            const precio = this.calcularPrecio(provincia, peso, tarifasPorPeso[tarifa]);
            importe = precio * cantidad;
         }

         if (telefonodestinatario && telefonodestinatario.length > 1) {
            importe += 2;
         }
