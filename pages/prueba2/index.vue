               ...
               <div class="col-12 col-sm-7">
                  <div class="row">
                     <div class="col-12 mb-3">
                        <button class="btn btn-primary" @click="mostrarSeccion('servicios')">Mostrar Servicios</button>
                        <button class="btn btn-secondary" @click="mostrarSeccion('casillas')">Mostrar Casillas</button>
                     </div>

                     <div v-if="seccionActiva === 'servicios'">
                        <div class="col-12">
                        ...
                        <div class="col-12 py-2" style="min-height: 60vh; max-height: 60vh; overflow-y: scroll;">
                           <div class="row">
                              <div class="col-6 col-md-4 col-lg-3 mb-3" v-for="m in servicios" :key="m.id">
                                 <PostServicio :servicio="m" @AddCarrito="() => AddCarrito(m, 'servicio')">
                                 </PostServicio>
                              </div>
                           </div>
                        </div>
                     ...
                     <div v-if="seccionActiva === 'casillas'">
                        <div class="col-12">
                           <div class="card bg-gradient-dark">
                              <div class="card-header bg-transparent py-2 px-3">
                                 <div class="row">
                        ...
                        <div class="col-12 py-2" style="min-height: 60vh; max-height: 60vh; overflow-y: scroll;">
                           <div class="row">
                              <div class="col-6 col-md-4 col-lg-3 mb-3" v-for="m in casillasConAlquiler" :key="m.id">
                                 <PostCasillas :casilla="m" @AddCarrito="() => AddCarrito(m, 'casilla')"></PostCasillas>
                              </div>
                           </div>
                        </div>
                     </div>
...
export default {
   ...
   methods: {
      ...
      AddCarrito(item, tipo) {
         let id = item.id
         let buscarRegistro = this.carrito.filter((i) => i.item.id == id && i.tipo === tipo);
         if (buscarRegistro.length > 0) {
            let indice = this.carrito.findIndex((i) => i.item.id == id && i.tipo === tipo);
            ...
            const carritoItem = {
               item: item,
               cantidad: 1,
               precio: tipo === 'servicio' ? item.precioUnitario : item.precioAlquiler, // Precio específico para servicio o casilla
               tipo: tipo
            };
            this.carrito.push(carritoItem);
            this.item = carritoItem;
            this.modalEdit = true;
         }
      },
   ...
   mounted() {
      this.$nextTick(async () => {
         try {
            await this.Datos();
            await this.CASILLAS('todas-las-casillas');
         } catch (e) {
